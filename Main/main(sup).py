...
def sup_train(train_loader, model, optimizer, device, gamma_joao, lamda, use_unsup_loss):
    model.train()
    total_loss = 0
    ...
    for data_tup1 in train_loader:
        ...
        loss = F.binary_cross_entropy(out, data.y.to(torch.float32)) + \
               model.loss_graphcl(out1, out2) * lamda if use_unsup_loss else \
            F.binary_cross_entropy(out, data.y.to(torch.float32))
        loss.backward()
        optimizer.step()
        total_loss += loss.item() * data1.num_graphs

    aug_prob_train = joao(train_loader, model, gamma_joao)
    return total_loss / len(train_loader.dataset), aug_prob_train
...
if __name__ == '__main__':
    ...
    for run in range(runs):
        ...
        for epoch in range(1, epochs + 1):
            ...
            _,  aug_prob_train = sup_train(train_loader, model, optimizer, device, gamma_joao, lamda, use_unsup_loss)
