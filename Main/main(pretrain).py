...
def pre_train(dataloader, model, optimizer, device, gamma_joao):
    ...
    aug_prob = joao(dataloader, model, gamma_joao)
    return total_loss / len(dataloader.dataset), aug_prob
...
if __name__ == '__main__':
    ...
    for run in range(runs):
        ...
        for epoch in range(1, epochs + 1):
            pretrain_loss, aug_prob = pre_train(unsup_train_loader, model, optimizer, device, gamma_joao)
            unsup_train_loader.dataset.set_aug_prob(aug_prob)

            log_info = 'Epoch: {:03d}, Loss: {:.7f}'.format(epoch, pretrain_loss)
            write_log(log, log_info)
            pr_aug_prob = [round(prob, 2) for prob in aug_prob]
            write_log(log, f'Aug Prob: {pr_aug_prob}')
